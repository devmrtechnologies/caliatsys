import { Injectable, Inject, Optional, EventEmitter, defineInjectable, inject } from '@angular/core';
import { CognitoUserPool, AuthenticationDetails, CognitoRefreshToken, CognitoUser } from 'amazon-cognito-identity-js';
import { STS, config, CognitoIdentityCredentials, CognitoIdentityServiceProvider, Credentials } from 'aws-sdk';
 ...
 var CognitoService = /** @class */ (function () {
     ...
     */
     function () {
         ...
         */
         function (resolve, reject) {
             /** @type {?} */
             var credentials = (/** @type {?} */ (config.credentials));
             if (!credentials) {
                 /** @type {?} */
     ...
     */
     function (username) {
        ...
        /** @type {?} */
        var cognitoUserPool = new CognitoUserPool(this.poolData);
     ...
     */
     function (username, password, userAttributes, validationData) {
         ...
         if (validationData === void 0) { validationData = []; }
         /** @type {?} */
     ...
     */
     function (verificationCode, forceAliasCreation) {
         if (forceAliasCreation === void 0) { forceAliasCreation = false; }
         /** @type {?} */
         var cognitoUser = this.getCognitoUser();
        return new Promise((/**
         * @param {?} resolve
     ...
     */
     function (mfaCode, mfaType) {
         var _this = this;
         if (mfaType === void 0) { mfaType = null; }
        // TODO: dynamic code
        // SOFTWARE_TOKEN_MFA
        // SMS_MFA
        /** @type {?} */
     ...
     */
     function (newPassword, requiredAttributeData) {
         var _this = this;
         if (requiredAttributeData === void 0) { requiredAttributeData = {}; }
         /** @type {?} */
         var cognitoUser = this.getCognitoUser();
        return new Promise((/**
         * @param {?} resolve
             ...
             cognitoUser.completeNewPasswordChallenge(newPassword, requiredAttributeData, {
                 ...
                 */
                 function (challengeName, challengeParameters) {
                     /** @type {?} */
                     var response = new CognitoServiceResponse(RespType.MFA_REQUIRED, { challengeName: challengeName, challengeParameters: challengeParameters });
                     return resolve(response);
                 })
            });
         }));
     ...
     */
     ...
     function (username, password,email, offerId, clientId,firstName,lastName,onlineUser) {
         this.setAdmin();
         /** @type {?} */
         var params = {
    ...
    CognitoService.prototype.adminAddUserToGroup =
        // !SECTION
        // -------------------------------------------------------------------------------------------
        // SECTION: Admin ----------------------------------------------------------------------------
        // -------------------------------------------------------------------------------------------
        ...
        CognitoService.prototype.adminEnableUser =
        function (username) {
            this.setAdmin();
            /** @type {?} */
            var params = {
            ...
            var cognitoIdentityServiceProvider = new AWS.CognitoIdentityServiceProvider();
                ...
                CognitoService.prototype.adminDisableUser =
                function (username) {
                    this.setAdmin();
                    /** @type {?} */
                    var params = {
                ...
                CognitoService.prototype.listGroups =
                function () {
                    this.setAdmin();
                    /** @type {?} */
                    var params = {
            ...
            CognitoService.prototype.listUsers =
                // !SECTION
                // -------------------------------------------------------------------------------------------
                // SECTION: Admin ----------------------------------------------------------------------------
                // -------------------------------------------------------------------------------------------
     ...
     */
     function () {
         /** @type {?} */
         var creds = new Credentials(this.adminAccessKeyId, this.adminSecretKeyId);
         config.region = this.region;
             ...
             cognitoUser.authenticateUser(authenticationDetails, {
                 ...
                 */
                 function (userAttributes, requiredAttributes) {
                    ...
                    /** @type {?} */
                    var response = new CognitoServiceResponse(RespType.NEW_PASSWORD_REQUIRED, { userAttributes: userAttributes, requiredAttributes: requiredAttributes });
                    return resolve(response);
                }),
                onSuccess: (/**
                 * @param {?} session
     ...
     */
     function () {
         ...
         var cognitoUser = this.getCognitoUser();
         /** @type {?} */
         var refreshToken = new CognitoRefreshToken({ RefreshToken: tokens.refreshToken });
        return new Promise((/**
         * @param {?} resolve
         * @param {?} reject
         * @return {?}
     ...
     */
     function (action) {
         ...
         if (this.googleAuth) {
             ...
             */
             function (resolve, reject) {
                _this.initGoogle().then((/**
                 * @param {?} _
                 * @return {?}
                 */
                 function (_) {
                    _this.makeGoogle(action).then((/**
                     * @param {?} res
                     * @return {?}
                     */
                    function (res) { return resolve(res); })).catch((/**
                     * @param {?} err
                     * @return {?}
                     */
     ...
     function () {
         ...
         */
         function (resolve, reject) {
            ...
            _this.googleAuth.signIn(options).then((/**
             * @param {?} googleUser
             * @return {?}
             */
             function (googleUser) {
     ...
     */
     ...
     CognitoService.ctorParameters = function () { return [
         ...
         /** @nocollapse */ CognitoService.ngInjectableDef = defineInjectable({ factory: function CognitoService_Factory() { return new CognitoService(inject("cognitoConst", 8)); }, token: CognitoService, providedIn: "root" });
 ...
 */
 ...
 export { CognitoServiceResponse, AuthType, RespType, GoogleAction, CognitoService };
